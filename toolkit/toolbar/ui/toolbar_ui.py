...
class Toolbar(QMainWindow):
                ...
                try:
                    ...
                    if hasattr(plugin, "get_ui"):
                        ...
                        if ui:
                            ...
                            # Add plugin tab if it has a widget
                            if hasattr(ui, "get_widget"):
    ...
    def _position_toolbar(self):
        ...
        try:
            ...
            if position == "top":
                ...
                self.move(screen.width() // 2 - self.width() // 2, screen.height() - self.height())
            elif position == "left":
                self.move(0, screen.height() // 2 - self.height() // 2)
            elif position == "right":
                self.move(screen.width() - self.width(), screen.height() // 2 - self.height() // 2)
            else:
                # Default to center
                self.move(screen.width() // 2 - self.width() // 2, screen.height() // 2 - self.height() // 2)
            
            logger.info(f"Toolbar positioned at {position}")
        except Exception as e:
    ...
    def _auto_save(self):
        ...
        try:
            self.config.save()
            logger.info("Configuration auto-saved")
        except Exception as e:
    ...
    def _repo_selected(self, index):
        ...
        try:
            ...
            # Update PR list
            self.pr_list.clear()
            if repo and "prs" in repo:
                for pr in repo["prs"]:
                    pr_item = QListWidgetItem(f"#{pr['number']} - {pr['title']}")
                    pr_item.setData(Qt.UserRole, pr)
