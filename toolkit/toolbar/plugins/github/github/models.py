...
class GitHubProject:
    ...
    def __init__(self, name, full_name, owner, url, icon_url=None):
        ...
        self.pinned = False
        self.show_pr_notifications = True
        self.show_branch_notifications = True
    
    def add_notification(self, notification: GitHubNotification) -> None:
        """Add a notification to this project."""
        # Check if we should add this notification based on settings
        if notification.type == 'pr' and not self.show_pr_notifications:
            return
        if notification.type == 'branch' and not self.show_branch_notifications:
            return
            
        # Check for duplicates
        for existing in self.notifications:
            if existing.url == notification.url and existing.type == notification.type:
                return
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'pinned': self.pinned,
            'show_pr_notifications': self.show_pr_notifications,
            'show_branch_notifications': self.show_branch_notifications,
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'GitHubProject':
        ...
        project.show_pr_notifications = data.get('show_pr_notifications', True)
        project.show_branch_notifications = data.get('show_branch_notifications', True)
